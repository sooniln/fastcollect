...
/**
 * A [HashMap](https://en.wikipedia.org/wiki/Hash_table) implementation for storing Long to Short
 * relationships.
 *
 ...
 */
public class Long2ShortHashMap @JvmOverloads constructor(
    capacity: Int = 0,

    /** The default value should be the value that is ideally least likely to occur in the map. */
    private val defaultValue: Short = Short.MIN_VALUE,

) : AbstractMutableLong2ShortMap() {

    public constructor(map: Long2ShortMap): this() { putAll(map) }
    public constructor(map: Map<Long, Short>): this() { putAll(map) }
    ...
    override fun isDefaultValue(value: Short): Boolean = value equalsBoxed defaultValue
    ...
    override fun containsValue(value: Short): Boolean {
        val keysArr = keysArr
    ...
    override fun get(key: Long): Short = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getValue(key: Long): Short = findSlot(key, { slot -> valuesArr[slot] as Short }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getOrDefault(key: Long, defaultValue: Short): Short = findSlot(key, { slot -> valuesArr[slot] as Short }, { defaultValue })
    ...
    public fun putIfAbsent(key: Long, value: Short): Short {
        set(key, { value }, { slot -> return valuesArr[slot] })
        return defaultValue
    }

    override fun set(key: Long, value: Short) {
        set(key, { value }, { value })
    }

    override fun replace(key: Long, value: Short): Short {
        var returnValue = defaultValue
        set(key, {
    ...
    override fun removeKey(key: Long): Short {
        return findSlot(
            key,
            { slot ->
                val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Long, onAdd: () -> Short, onReplace: (slot: Int) -> Short) {
    ...
    private fun shiftAndInsert(keysArr: LongArray, mask: Int, slot: Int, currKey: Long, newKey: Long, newValue: Short) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Long2ShortMap) {
        if (from is Long2ShortHashMap && from.size / 2 > size) {
            ...
            for (slot in oldKeysArr.indices) {
                ...
                if (key != oldEmptyKey) {
                    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
                    putIfAbsent(key, oldValuesArr[slot] as Short)
                }
            }
            trimToSize()
        } else {
    ...
    override fun putAll(from: Map<out Long, Short>) {
        ensureCapacity(max(size + (from.size / 2), from.size))
        for ((key, value) in from) {
            set(key, value)
        }
    ...
    private fun resetTo(from: Long2ShortHashMap) {
        check(!from.isEmpty())
    ...
    override val keys: MutableLongSet get() {
        return _keys ?:
            object : MutableLongSet {
                override val size: Int get() = this@Long2ShortHashMap.size
    ...
    private var _values: MutableShortCollection? = null
    override val values: MutableShortCollection get() {
        return _values ?:

            object : MutableShortCollection {

                override val size: Int get() = this@Long2ShortHashMap.size
                override fun contains(element: Short): Boolean = containsValue(element)
                override fun add(element: Short): Boolean = throw UnsupportedOperationException()
                override fun remove(element: Short): Boolean = throw UnsupportedOperationException()
                override fun iterator(): MutableShortIterator = ValueIterator()

                override fun foreach(action: ShortConsumer) = foreach { _, value -> action.accept(value) }
    ...
    private fun rehash(capacity: Int) {
        ...
        val newValuesArr = ShortArray(newLength)
    ...
    private fun setRehashing(keysArr: LongArray, valuesArr: ShortArray, mask: Int, key: Long, value: Short) {
    ...
    override operator fun iterator(): MutableFastIterator<MutableLong2ShortMap.MutableEntry> = FastEntryIterator()


    override fun foreach(action: LongShortConsumer) {
        ...
        for (slot in keysArr.indices) {
            ...
            if (key != emptyKey) {
                @Suppress("UNCHECKED_CAST", "USELESS_CAST")
                action.accept(key, valuesArr[slot] as Short)
            }
        }
    }
    ...
    private open inner class SlotIterator {
        private val keysArr = this@Long2ShortHashMap.keysArr
        private val valuesArr = this@Long2ShortHashMap.valuesArr
        private val emptyKey = this@Long2ShortHashMap.emptyKey
        ...
        @Suppress("UNCHECKED_CAST", "USELESS_CAST")
        fun value(): Short = valuesArr[previousSlot] as Short

        fun updateValue(newValue: Short) {
            check(previousSlot != -1)
            if (keysArr !== this@Long2ShortHashMap.keysArr) throw ConcurrentModificationException()
    ...
    private inner class ValueIterator : MutableShortIterator() {
    ...
    private inner class FastEntryIterator: SlotIterator(), MutableFastIterator<MutableLong2ShortMap.MutableEntry>, MutableLong2ShortMap.MutableEntry {
        ...
        override fun next(): MutableLong2ShortMap.MutableEntry {
            nextSlot()
            return this
        }

        override fun equals(other: Any?): Boolean = other is Long2ShortMap.Entry && other.key equalsBoxed key && other.value equalsBoxed value
    ...
    private companion object {
        ...
        private val EMPTY_VALUE_ARRAY = ShortArray(1)

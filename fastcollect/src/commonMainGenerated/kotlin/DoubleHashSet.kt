...
/**
 * A [HashSet](https://en.wikipedia.org/wiki/Hash_table) implementation for storing Doubles.
 *
...
public class DoubleHashSet @JvmOverloads constructor(
    capacity: Int = 0,
) : AbstractMutableDoubleSet() {
    ...
    @JvmOverloads
    public constructor(array: DoubleArray, fromIndex: Int = 0, toIndex: Int = array.size): this() { addAll(array, fromIndex, toIndex) }

    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var keysArr: DoubleArray = EMPTY_ARRAY

    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var emptyKey: Double = ZERO
    ...
    override fun contains(element: Double): Boolean = findSlot(element, { true }, { false })

    override fun add(element: Double): Boolean = add(element, { false }, { true })
    ...
    private inline fun <T> findSlot(key: Double, onFind: (slot: Int) -> T, onFail: () -> T): T {
        val keysArr = keysArr
    ...
    private inline fun <T> add(key: Double, onPresent: () -> T, onAbsent: () -> T): T {
    ...
    private fun shiftAndInsert(keysArr: DoubleArray, mask: Int, slot: Int, currKey: Double, newKey: Double) {
        var nextSlot = slot
        var currKey = currKey
        var newKey = newKey
    ...
    override fun addAll(elements: DoubleCollection): Boolean {
        val oldSize = size
        if (elements is DoubleHashSet && elements.size / 2 > size) {
    ...
    override fun addAll(array: DoubleArray, fromIndex: Int, toIndex: Int): Boolean {
    ...
    private fun resetTo(elements: DoubleHashSet) {
    ...
    private fun rehash(capacity: Int) {
        ...
        val newKeysArr = DoubleArray(newLength)
    ...
    private fun addRehashing(keysArr: DoubleArray, mask: Int, key: Double) {
        var slot = key.slot(mask)
        var distance = 0
        while (true) {
            var currKey = keysArr[slot]
    ...
    private fun changeEmptyKey() {
        ...
        while (candidate equalsRaw emptyKey || contains(candidate)) {
            ...
            candidate = Double.fromBits(Random.nextLong())
    ...
    private fun Double.slot(mask: Int): Int = Hash.mix(this) and mask
    private fun Double.slotDistance(slot: Int, mask: Int): Int = (slot - Hash.mix(this)) and mask

    private companion object {
        // the value of a field in an uninitialized primitive array
        @Suppress("REDUNDANT_CALL_OF_CONVERSION_METHOD")
        private const val ZERO: Double = 0.toDouble()

        private val EMPTY_ARRAY = doubleArrayOf(ZERO)

        private const val MIN_INITIAL_CAPACITY = 7

        private const val CACHE_LINE_SIZE = 64 / Double.SIZE_BYTES
...
public fun DoubleHashSet.removeAll(predicate: DoublePredicate): Boolean = filterInPlace { predicate.test(it) }
public fun DoubleHashSet.retainAll(predicate: DoublePredicate): Boolean = filterInPlace { !predicate.test(it) }

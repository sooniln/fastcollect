...
public class LongArrayDeque private constructor(array: LongArray, size: Int = array.size) : AbstractMutableLongList(), RandomAccess {
    ...
    private fun addInternal(position: Int, element: Long): Int {
        val newSize = size + 1
        ensureCapacity(newSize)
    ...
    override fun traverser(): MutableLongTraverser = ListTraverser(0)
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other is RandomAccess) {
            ...
            var i = 0
            while (it.hasNext()) {
                if (!(it.nextLong() equalsBoxed this[i++])) return false
            }
        }
        return true
    }
    ...
    private inner class ListTraverser(position: Int) : MutableLongListTraverser {
        private val ring = this@LongArrayDeque.ring
        private var head: Int = this@LongArrayDeque.head

...
public class LongArrayDeque private constructor(array: LongArray, size: Int) : AbstractMutableLongList() {
    ...
    public constructor(elements: LongCollection) : this(elements.toArray(), size = elements.size)

    public constructor(elements: Collection<Long>) : this(elements.toLongArray(), size = elements.size)

    public constructor(elements: LongArray) : this(elements.copyOf(), size = elements.size)

    public constructor(elements: LongArray, fromIndex:Int, toIndex: Int) : this(elements.copyOfRange(fromIndex, toIndex), size = toIndex - fromIndex)
    ...
    private fun grow(capacity: Int) {
        ...
        if (head == 0) {
            ...
            ring = copyIntoInternal(LongArray(newCapacity))
            head = 0
        }
    }
    ...
    public fun trimToSize() {
        if (size < ring.size) {
            ring = if (isEmpty()) EMPTY_ARRAY else copyIntoInternal(LongArray(size))
            head = 0
        }
    }
    ...
    override fun addAll(array: LongArray, fromIndex: Int, toIndex: Int): Boolean {
    ...
    public override fun removeAll(elements: LongCollection): Boolean {
    ...
    public override fun retainAll(elements: LongCollection): Boolean {
        return filterInPlace { e -> !elements.contains(e) }
    }
    ...
    private fun copyIntoInternal(dest: LongArray, destinationOffset: Int = 0, fromIndex: Int = 0, toIndex: Int = size): LongArray {
    ...
    internal companion object {
        ...
        private fun LongArray.position(head: Int, index: Int): Int = positiveMod(head + index)

        private fun LongArray.index(head: Int, position: Int): Int = negativeMod(position - head)

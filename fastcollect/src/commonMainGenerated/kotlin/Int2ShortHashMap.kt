...
/**
 * A [HashMap](https://en.wikipedia.org/wiki/Hash_table) implementation for storing Int to Short
 * relationships.
 *
 ...
 */
public class Int2ShortHashMap @JvmOverloads constructor(
    capacity: Int = 0,
...
) : AbstractMutableInt2ShortMap() {

    public constructor(map: Int2ShortMap): this() { putAll(map) }
    public constructor(map: Map<Int, Short>): this() { putAll(map) }
    ...
    override fun get(key: Int): Short = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getValue(key: Int): Short = findSlot(key, { slot -> valuesArr[slot] as Short }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getOrDefault(key: Int, defaultValue: Short): Short = findSlot(key, { slot -> valuesArr[slot] as Short }, { defaultValue })
    ...
    public fun putIfAbsent(key: Int, value: Short): Short {
        set(key, { value }, { slot -> return valuesArr[slot] })
        return defaultValue
    }

    override fun set(key: Int, value: Short) {
        set(key, { value }, { value })
    }

    override fun replace(key: Int, value: Short): Short {
        var returnValue = defaultValue
        set(key, {
    ...
    override fun removeKey(key: Int): Short {
        return findSlot(
            key,
            { slot ->
                val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Int, onAdd: () -> Short, onReplace: (slot: Int) -> Short) {
    ...
    private fun shiftAndInsert(keysArr: IntArray, mask: Int, slot: Int, currKey: Int, newKey: Int, newValue: Short) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Int2ShortMap) {
        if (from is Int2ShortHashMap && from.size / 2 > size) {
    ...
    override fun putAll(from: Map<out Int, Short>) {
        ensureCapacity(max(size + (from.size / 2), from.size))
        for ((key, value) in from) {
            set(key, value)
        }
    ...
    private fun resetTo(from: Int2ShortHashMap) {
        check(!from.isEmpty())
    ...
    override val keys: MutableIntSet get() {
        return _keys ?:
            object : MutableIntSet {
                override val size: Int get() = this@Int2ShortHashMap.size
    ...
    private fun setRehashing(keysArr: IntArray, valuesArr: ShortArray, mask: Int, key: Int, value: Short) {
    ...
    override operator fun iterator(): MutableFastIterator<MutableInt2ShortMap.MutableEntry> = FastEntryIterator()


    override fun foreach(action: IntShortConsumer) {
    ...
    private open inner class SlotIterator {
        private val keysArr = this@Int2ShortHashMap.keysArr
        private val valuesArr = this@Int2ShortHashMap.valuesArr
        private val emptyKey = this@Int2ShortHashMap.emptyKey
        ...
        fun updateValue(newValue: Short) {
            ...
            if (keysArr !== this@Int2ShortHashMap.keysArr) throw ConcurrentModificationException()
    ...
    private inner class FastEntryIterator: SlotIterator(), MutableFastIterator<MutableInt2ShortMap.MutableEntry>, MutableInt2ShortMap.MutableEntry {
        ...
        override fun next(): MutableInt2ShortMap.MutableEntry {
            nextSlot()
            return this
        }

        override fun equals(other: Any?): Boolean = other is Int2ShortMap.Entry && other.key equalsBoxed key && other.value equalsBoxed value

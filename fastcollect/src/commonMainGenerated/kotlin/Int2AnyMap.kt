...
public fun <V> int2AnyMapOf(key: Int, value: V): Int2AnyMap<V> = SingletonInt2AnyMap(key, value)
@JvmSynthetic
...
public fun <V> Int2AnyMap<V>.isNotEmpty(): Boolean = size != 0

@JvmSynthetic

public fun <A : Appendable, V> Int2AnyMap<V>.joinTo(buffer: A, separator: CharSequence = ", ", prefix: CharSequence = "", postfix: CharSequence = "", transform: ((Int2AnyMap.Entry<V>) -> CharSequence)? = null): A {
...
@JvmOverloads
public fun <V> Int2AnyMap<V>.joinToString(separator: CharSequence = ", ", prefix: CharSequence = "", postfix: CharSequence = ""): String {
...
public fun <V> Int2AnyMap.Entry<V>.asEntry(): Map.Entry<Int, V> = Int2AnyMapEntryWrapper(this)
...
public fun <V> MutableInt2AnyMap<V>.asMap(): MutableMap<Int, V> = MutableInt2AnyMapWrapper(this)

public fun <V> MutableInt2AnyMap.MutableEntry<V>.asEntry(): MutableMap.MutableEntry<Int, V> = MutableInt2AnyMapEntryWrapper(this)
...
public abstract class AbstractInt2AnyMap<V> : Int2AnyMap<V> {
    ...
    public class SimpleEntry<V>(override val key: Int, override val value: V) : Int2AnyMap.AbstractEntry<V>()
}
...
private class SingletonInt2AnyMap<V>(
    private val key: Int,
    private val value: V
) : AbstractInt2AnyMap<V>() {
    ...
    override fun get(key: Int): V? = if (key equalsRaw this.key) value else null
    ...
    override val entries: Set<Map.Entry<Int, V>> = object : AbstractSet<Map.Entry<Int, V>>() {
        ...
        override fun iterator(): Iterator<Map.Entry<Int, V>> = object : Iterator<Map.Entry<Int, V>> {
            ...
            override fun next(): Map.Entry<Int, V> {
                val entry = it.next()
                return AbstractInt2AnyMap.SimpleEntry(entry.key, entry.value).asEntry()
            }
        }
    }
}

private class Int2AnyMapEntryWrapper<V>(
    private val entry: Int2AnyMap.Entry<V>
) : Map.Entry<Int, V> {
        ...
        override fun iterator(): MutableIterator<MutableMap.MutableEntry<Int, V>> = object : MutableIterator<MutableMap.MutableEntry<Int, V>> {
            ...
            override fun next(): MutableMap.MutableEntry<Int, V> {
                ...
                return object : MutableInt2AnyMap.AbstractMutableEntry<V>() {
                    ...
                    override var value: V = entryValue
                        set(value) {
                            map.put(key, value)
                            field = value
                        }
...
private class MutableInt2AnyMapEntryWrapper<V>(
    private val entry: MutableInt2AnyMap.MutableEntry<V>

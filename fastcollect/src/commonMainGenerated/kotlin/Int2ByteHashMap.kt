...
public class Int2ByteHashMap @JvmOverloads constructor(
    ...
    override fun getValue(key: Int): Byte = findSlot(key, { slot -> valuesArr[slot] }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST")
    override fun getOrDefault(key: Int, defaultValue: Byte): Byte = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the Int2ByteHashMap while this
     * method is ongoing is not supported and leads to undefined behavior.
     */
    @JvmSynthetic
    public inline fun forEach(action: (Int2ByteMap.Entry) -> Unit) {
        val keysArr = keysArr
    ...
    private inner class MutableReusableEntry : MutableInt2ByteMap.AbstractMutableEntry() {
    ...
    private inner class EntryIterator : MutableIterator<MutableInt2ByteMap.MutableEntry> {
        private val entry = MutableReusableEntry()

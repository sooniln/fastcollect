...
public fun intPriorityQueueOf(vararg elements: Int): IntPriorityQueue {
    return IntPriorityQueue(elements, descending = false)
}

public fun intDescendingPriorityQueueOf(vararg elements: Int): IntPriorityQueue {
    return IntPriorityQueue(elements, descending = true)
}
...
public abstract class AbstractIntPriorityQueue(initialCapacity: Int): IntCollection {
     ...
     * heap at all.
     */
    protected open fun onRemoved(element: Int, index: Int) {}
    ...
    final override fun contains(element: Int): Boolean {
        for (i in 0..<size) {
            if (heap[i] equalsRaw element) {
                return true
            }
    ...
    final override fun copyInto(destination: IntArray, destinationOffset: Int): IntArray {
        destination.rangeCheck(destinationOffset, destinationOffset + size)
        return heap.copyInto(destination, destinationOffset, 0, size)
    }

    final override fun iterator(): IntIterator = object : IntIterator() {
        private val size = this@AbstractIntPriorityQueue.size
        private var index = 0
        override fun hasNext(): Boolean = index < size
        override fun nextInt(): Int {
            if (!hasNext()) throw NoSuchElementException()
            if (size != this@AbstractIntPriorityQueue.size) throw ConcurrentModificationException()
            return heap[index++]
        }
    }

    final override fun traverser(): IntTraverser = object : IntTraverser {
 ...
 */
public class IntPriorityQueue(private val descending: Boolean) : AbstractIntPriorityQueue() {
    ...
    @JvmOverloads
    public constructor(array: IntArray, fromIndex: Int = 0, toIndex: Int = array.size, descending: Boolean = false) : this(descending) {
        addAll(array, fromIndex, toIndex)
    }

    @JvmOverloads
    public constructor(elements: IntCollection, descending: Boolean = false) : this(descending) {
        addAll(elements)
    }

    @JvmOverloads
    public constructor(elements: Collection<Int>, descending: Boolean = false) : this(descending) {
        addAll(elements)
    }
...
/**
 * Returns a priority queue of Ints using the given priority function.
 */
@JvmSynthetic

...
public inline fun buildDoublePriorityQueue(
    expectedSize: Int = 0,
    descending: Boolean = false,
    builderAction: DoublePriorityQueue.() -> Unit,
): DoublePriorityQueue {
    contract { callsInPlace(builderAction, InvocationKind.EXACTLY_ONCE) }

    val priorityQueue = DoublePriorityQueue(expectedSize, descending)
    priorityQueue.builderAction()
    return priorityQueue
}

/**
 * An abstract base class for priority queues of Doubles, backed by a binary heap. Subclasses can override
...
public abstract class AbstractDoublePriorityQueue(initialCapacity: Int = 0) {
    ...
    public constructor(elements: Collection<Double>) : this(elements.size) {
        addAll(elements)
    }
...
public fun AbstractDoublePriorityQueue.removeAll(predicate: DoublePredicate): Boolean = filterInPlace { predicate.test(it) }
public fun AbstractDoublePriorityQueue.retainAll(predicate: DoublePredicate): Boolean = filterInPlace { !predicate.test(it) }

/**
 * A priority queue of Doubles, backed by a binary heap, which can operate in descending (largest element at the front
...
public inline fun DoublePriorityQueue(
    ...
    return object : AbstractDoublePriorityQueue(initialCapacity) {
        override fun isHigherPriority(element1: Double, element2: Double): Boolean = isHigherPriority(element1, element2)
    }
}

...
/**
 * An array based [Deque](https://en.wikipedia.org/wiki/Double-ended_queue) implementation for storing Floats.
 *
...
public class FloatArrayDeque private constructor(array: FloatArray, size: Int = array.size) : AbstractMutableFloatList(), RandomAccess {
    ...
    private fun grow(capacity: Int) {
        ...
        if (head == 0) {
            ...
            ring = copyIntoInternal(FloatArray(newCapacity))
            head = 0
        }
    }
    ...
    public fun trimToSize() {
        if (size < ring.size) {
            ring = if (isEmpty()) EMPTY_ARRAY else copyIntoInternal(FloatArray(size))
            head = 0
        }
    }
    ...
    override fun addFirst(element: Float) {
        val newSize = size + 1
    ...
    override fun addLast(element: Float) {
        val s = size
        val newSize = s + 1
        ensureCapacity(newSize)
    ...
    private fun addMiddle(index: Int, element: Float) {
        val newSize = size + 1
        ensureCapacity(newSize)
    ...
    override fun removeFirst(): Float {
    ...
    private fun indexOfContinuous(tail: Int, element: Float): Int {
        for (i in head..<tail) {
    ...
    private fun indexOfDiscrete(tail: Int, element: Float): Int {
    ...
    public override fun removeAll(elements: FloatCollection): Boolean {
    ...
    public override fun retainAll(elements: FloatCollection): Boolean {
        return filterInPlace { e -> !elements.contains(e) }
    }
    ...
    private fun copyIntoInternal(dest: FloatArray, destinationOffset: Int = 0, fromIndex: Int = 0, toIndex: Int = size): FloatArray {
    ...
    override fun traverser(position: Int): MutableFloatListTraverser = ListTraverser(position)
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other is RandomAccess) {
            ...
            var i = 0
            while (it.hasNext()) {
                if (!(it.nextFloat() equalsRaw this[i++])) return false
            }
        }
        return true
    }
    ...
    private inner class ContinuousIterator(private var tail: Int) : MutableFloatIterator() {
    ...
    private inner class DiscreteIterator : MutableFloatIterator() {
        private val ring = this@FloatArrayDeque.ring
    ...
    private inner class ContinuousTraverser(tail: Int) : MutableFloatTraverser {
         private val ring = this@FloatArrayDeque.ring
     ...
     private inner class DiscreteTraverser : MutableFloatTraverser {
         private val ring = this@FloatArrayDeque.ring
    ...
    private inner class ListTraverser(position: Int) : MutableFloatListTraverser {
        init {
            indexCheckInclusive(position)
        }

        private var size = this@FloatArrayDeque.size
    ...
    internal companion object {
        ...
        private fun FloatArray.positiveMod(position: Int): Int = if (position < size) position else position - size

        private fun FloatArray.negativeMod(position: Int): Int = if (position < 0) position + size else position

        private fun FloatArray.position(head: Int, index: Int): Int = positiveMod(head + index)

        private fun FloatArray.index(head: Int, position: Int): Int = negativeMod(position - head)

        private fun FloatArray.incrementPosition(position: Int): Int {
            val next = position + 1
            return if (next == size) 0 else next
        }

        private fun FloatArray.decrementPosition(position: Int): Int = if (position == 0) size - 1 else position - 1
    }
}

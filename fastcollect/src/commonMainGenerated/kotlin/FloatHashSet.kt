...
/**
 * A [HashSet](https://en.wikipedia.org/wiki/Hash_table) implementation for storing Floats.
 *
...
public class FloatHashSet @JvmOverloads constructor(
    ...
    override fun contains(element: Float): Boolean = findSlot(element, { true }, { false })

    override fun add(element: Float): Boolean = add(element, { false }, { true })
    ...
    private inline fun <T> findSlot(key: Float, onFind: (slot: Int) -> T, onFail: () -> T): T {
        val keysArr = keysArr
    ...
    private fun shiftAndInsert(keysArr: FloatArray, mask: Int, slot: Int, currKey: Float, newKey: Float) {
        var nextSlot = slot
        var currKey = currKey
        var newKey = newKey
    ...
    private fun resetTo(elements: FloatHashSet) {
    ...
    private fun addRehashing(keysArr: FloatArray, mask: Int, key: Float) {
        var slot = key.slot(mask)
        var distance = 0
        while (true) {
            var currKey = keysArr[slot]
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the FloatHashSet while this
    ...
    private companion object {
        ...
        private const val ZERO: Float = 0.toFloat()

        private val EMPTY_ARRAY = floatArrayOf(ZERO)

        private const val MIN_INITIAL_CAPACITY = 7

        private const val CACHE_LINE_SIZE = 64 / Float.SIZE_BYTES

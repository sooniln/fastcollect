...
/**
 * A [HashSet](https://en.wikipedia.org/wiki/Hash_table) implementation for storing Longs.
 *
...
public class LongHashSet @JvmOverloads constructor(
    ...
    @JvmOverloads
    public constructor(array: LongArray, fromIndex: Int = 0, toIndex: Int = array.size): this() { addAll(array, fromIndex, toIndex) }

    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var keysArr: LongArray = EMPTY_ARRAY
    ...
    override fun addAll(elements: LongCollection): Boolean {
        val oldSize = size
        if (elements is LongHashSet && elements.size / 2 > size) {
...
public fun LongHashSet.removeAll(predicate: LongPredicate): Boolean = filterInPlace { predicate.test(it) }
public fun LongHashSet.retainAll(predicate: LongPredicate): Boolean = filterInPlace { !predicate.test(it) }

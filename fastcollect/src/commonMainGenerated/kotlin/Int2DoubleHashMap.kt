...
public class Int2DoubleHashMap @JvmOverloads constructor(
    ...
    @JvmOverloads
    public constructor(map: Int2DoubleMap, defaultValue: Double = Double.NaN): this(defaultValue = defaultValue) { putAll(map) }
    @JvmOverloads
    public constructor(map: Map<Int, Double>, defaultValue: Double = Double.NaN): this(defaultValue = defaultValue) { putAll(map) }
    ...
    override fun get(key: Int): Double = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getValue(key: Int): Double = findSlot(key, { slot -> valuesArr[slot] as Double }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getOrDefault(key: Int, defaultValue: Double): Double = findSlot(key, { slot -> valuesArr[slot] as Double }, { defaultValue })
    ...
    override fun putIfAbsent(key: Int, value: Double): Double {
    ...
    override fun replace(key: Int, value: Double): Double {
    ...
    override fun remove(key: Int, value: Double): Boolean {
            return findSlot(
                key,
                { slot ->
                    val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Int, onAdd: () -> Double, onReplace: (slot: Int) -> Double) {
    ...
    private fun shiftAndInsert(keysArr: IntArray, mask: Int, slot: Int, currKey: Int, newKey: Int, newValue: Double) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Int2DoubleMap) {
        if (from is Int2DoubleHashMap && from.size / 2 > size) {
    ...
    private fun resetTo(from: Int2DoubleHashMap) {
        check(!from.isEmpty())
    ...
    private fun setRehashing(keysArr: IntArray, valuesArr: DoubleArray, mask: Int, key: Int, value: Double) {
    ...
    override operator fun iterator(): MutableIterator<MutableInt2DoubleMap.MutableEntry> = EntryIterator()

    /** Guaranteed to be as fast or faster than using [iterator] to iterate. */
    @JvmSynthetic
    public inline fun forEach(action: (Int2DoubleMap.Entry) -> Unit) {
        val keysArr = keysArr
    ...
    private inner class MutableReusableEntry : MutableInt2DoubleMap.AbstractMutableEntry() {
        private val keysArr = this@Int2DoubleHashMap.keysArr
        private val emptyKey = this@Int2DoubleHashMap.emptyKey
    ...
    private inner class EntryIterator : MutableIterator<MutableInt2DoubleMap.MutableEntry> {
        private val entry = MutableReusableEntry()

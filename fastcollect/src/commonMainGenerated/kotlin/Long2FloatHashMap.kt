...
public class Long2FloatHashMap @JvmOverloads constructor(
    ...
    override fun get(key: Long): Float = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })

    @Suppress("UNCHECKED_CAST")
    override fun getValue(key: Long): Float = findSlot(key, { slot -> valuesArr[slot] }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST")
    override fun getOrDefault(key: Long, defaultValue: Float): Float = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })
    ...
    override fun putIfAbsent(key: Long, value: Float): Float {
    ...
    override fun replace(key: Long, value: Float): Float {
    ...
    override fun remove(key: Long, value: Float): Boolean {
            return findSlot(
                key,
                { slot ->
                    val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Long, onAdd: () -> Float, onReplace: (slot: Int) -> Float) {
    ...
    private fun shiftAndInsert(keysArr: LongArray, mask: Int, slot: Int, currKey: Long, newKey: Long, newValue: Float) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Long2FloatMap) {
        if (from is Long2FloatHashMap && from.size / 2 > size) {
    ...
    private fun resetTo(from: Long2FloatHashMap) {
        check(!from.isEmpty())
    ...
    private fun setRehashing(keysArr: LongArray, valuesArr: FloatArray, mask: Int, key: Long, value: Float) {
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the Long2FloatHashMap while this
     * method is ongoing is not supported and leads to undefined behavior.
     */
    @JvmSynthetic
    public inline fun forEach(action: (Long2FloatMap.Entry) -> Unit) {
        val keysArr = keysArr
    ...
    private inner class MutableReusableEntry : MutableLong2FloatMap.AbstractMutableEntry() {
        private val keysArr = this@Long2FloatHashMap.keysArr
        private val emptyKey = this@Long2FloatHashMap.emptyKey
    ...
    private inner class EntryIterator : MutableIterator<MutableLong2FloatMap.MutableEntry> {
        private val entry = MutableReusableEntry()

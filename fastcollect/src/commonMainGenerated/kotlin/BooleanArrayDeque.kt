...
public typealias BooleanArrayList = BooleanArrayDeque

/**
 * An array based [Deque](https://en.wikipedia.org/wiki/Double-ended_queue) implementation for storing Booleans. Can be
 ...
 */
public class BooleanArrayDeque private constructor(array: BooleanArray, size: Int = array.size) : AbstractMutableBooleanList(), RandomAccess {

    private var head: Int = 0
    private var ring: BooleanArray = array
    override var size: Int = size
        private set

    public constructor(capacity: Int = 0) : this(if (capacity == 0) EMPTY_ARRAY else BooleanArray(capacity), 0)

    public constructor(elements: BooleanCollection) : this(if (elements is BooleanList) elements.toBooleanArray() else elements.toBooleanArray())

    public constructor(elements: Collection<Boolean>) : this(if (elements is BooleanList) elements.toBooleanArray() else elements.toBooleanArray())

    public constructor(elements: BooleanArray, fromIndex:Int = 0, toIndex: Int = elements.size) : this(elements.copyOfRange(fromIndex, toIndex))
    ...
    private fun grow(capacity: Int) {
        ...
        if (head == 0) {
            ...
            ring = copyFromRing(BooleanArray(newCapacity))
            head = 0
        }
    }

    private fun copyFromRing(dest: BooleanArray): BooleanArray {
    ...
    public fun trimToSize() {
        if (size < ring.size) {
            ring = if (isEmpty()) EMPTY_ARRAY else copyFromRing(BooleanArray(size))
            head = 0
        }
    }
    ...
    override fun addFirst(element: Boolean) {
        val newSize = size + 1
    ...
    override fun addLast(element: Boolean) {
        val s = size
        val newSize = s + 1
        ensureCapacity(newSize)
    ...
    private fun addMiddle(index: Int, element: Boolean) {
        val newSize = size + 1
        ensureCapacity(newSize)
    ...
    private fun indexOfContinuous(tail: Int, element: Boolean): Int {
        for (i in head..<tail) {
    ...
    private fun indexOfDiscrete(tail: Int, element: Boolean): Int {
    ...
    private fun lastIndexOfContinuous(tail: Int, element: Boolean): Int {
    ...
    private fun lastIndexOfDiscrete(tail: Int, element: Boolean): Int {
        // kotlin produces inefficient bytecode for downTo for some reason, so we use a manual loop
        val head = head
    ...
    public fun addAll(elements: BooleanArrayDeque): Boolean {
    ...
    private fun addToRing(src: BooleanArray, fromIndex: Int, toIndex: Int) {
    ...
    override fun addAll(elements: BooleanCollection): Boolean {
        if (elements is BooleanArrayDeque) return addAll(elements)
        if (elements.isEmpty()) return false
    ...
    public override fun removeAll(elements: Collection<Boolean>): Boolean {
        return filterInPlace { e -> elements.contains(e) }
    }

    public override fun retainAll(elements: Collection<Boolean>): Boolean {
        return filterInPlace { e -> !elements.contains(e) }
    }

    @OptIn(ExperimentalContracts::class)
    internal inline fun filterInPlace(removePredicate: (Boolean) -> Boolean): Boolean {
        contract {
    ...
    override fun toBooleanArray(): BooleanArray {
        return copyFromRing(BooleanArray(size))
    }
    ...
    override fun foreach(action: BooleanConsumer) {
        val ring = ring
        val tail = head + size
        if (tail > ring.size) {
    ...
    private fun foreachContinuous(ring: BooleanArray, tail: Int, action: BooleanConsumer) {
        var position = head
        while (position < tail) {
    ...
    private fun foreachDiscrete(ring: BooleanArray, action: BooleanConsumer) {
        var remaining = size
        var position = head
        while (remaining > 0) {
            action.accept(ring[position])
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other is RandomAccess) {
            ...
            var i = 0
            while (it.hasNext()) {
                if (!(it.nextBoolean() equalsBoxed this[i++])) return false
            }
        }
        return true
    }
    ...
    private inner class ContinuousIterator(private var tail: Int) : MutableBooleanIterator() {
        private val ring = this@BooleanArrayDeque.ring
        ...
        override fun remove() {
            if (ring !== this@BooleanArrayDeque.ring) throw ConcurrentModificationException()
    ...
    private inner class DiscreteIterator : MutableBooleanIterator() {
        private val ring = this@BooleanArrayDeque.ring
    ...
    private fun BooleanArray.positiveMod(position: Int): Int = if (position < size) position else position - size

    private fun BooleanArray.negativeMod(position: Int): Int = if (position < 0) position + size else position

    private fun BooleanArray.position(index: Int): Int = positiveMod(head + index)

    private fun BooleanArray.index(position: Int): Int = negativeMod(position - head)

    private fun BooleanArray.incrementPosition(position: Int): Int {
        val next = position + 1
        return if (next == size) 0 else next
    }

    private fun BooleanArray.decrementPosition(position: Int): Int = if (position == 0) size - 1 else position - 1

    internal companion object {
        private val EMPTY_ARRAY = BooleanArray(0)
        private const val DEFAULT_CAPACITY = 8

        fun wrap(array: BooleanArray): BooleanArrayDeque = BooleanArrayDeque(array)
    }
}

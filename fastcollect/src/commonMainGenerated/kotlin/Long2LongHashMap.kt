...
public class Long2LongHashMap @JvmOverloads constructor(
    ...
    @JvmOverloads
    public constructor(map: Long2LongMap, defaultValue: Long = Long.MIN_VALUE): this(defaultValue = defaultValue) { putAll(map) }
    @JvmOverloads
    public constructor(map: Map<Long, Long>, defaultValue: Long = Long.MIN_VALUE): this(defaultValue = defaultValue) { putAll(map) }
    ...
    override fun isDefaultValue(value: Long): Boolean = value equalsRaw defaultValue
    ...
    override fun getValue(key: Long): Long = findSlot(key, { slot -> valuesArr[slot] }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST")
    override fun getOrDefault(key: Long, defaultValue: Long): Long = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })
    ...
    private var _values: LongCollection? = null

    @get:JvmName("values")
    override val values: LongCollection get() {
        return _values ?:

            object : AbstractLongCollection() {
    ...
    override operator fun iterator(): MutableIterator<MutableLong2LongMap.MutableEntry> = EntryIterator()

    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the Long2LongHashMap while this
     * method is ongoing is not supported and leads to undefined behavior.
     */
    @JvmSynthetic
    public inline fun forEach(action: (Long2LongMap.Entry) -> Unit) {
        val keysArr = keysArr
    ...
    ) : Long2LongMap.AbstractEntry() {
        @set:JvmSynthetic
        override var key: Long = 0.toLong()
        @Suppress("UNCHECKED_CAST")
        override val value: Long get() = valuesArr[slot]
    }

    private inner class MutableReusableEntry : MutableLong2LongMap.AbstractMutableEntry() {
    ...
    private inner class EntryIterator : MutableIterator<MutableLong2LongMap.MutableEntry> {
        private val entry = MutableReusableEntry()

...
public class IntArrayDeque private constructor(array: IntArray, size: Int) : AbstractMutableIntList() {
    ...
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var ring: IntArray = array
    ...
    @JvmSynthetic
    public inline fun forEachReverse(action: (Int) -> Unit) {
        val ring = ring
        val tail = head + size
        if (tail > ring.size) {
    ...
    private inner class ListIteratorImpl(private var position: Int) : MutableIntListIterator() {
        ...
        private var ring = this@IntArrayDeque.ring
        ...
        override fun hasNext(): Boolean = position < this@IntArrayDeque.size

        override fun nextInt(): Int {
            if (position >= this@IntArrayDeque.size) throw NoSuchElementException()
            if (ring !== this@IntArrayDeque.ring) throw ConcurrentModificationException()
    ...
    internal companion object {
        ...
        @JvmSynthetic
        internal fun wrap(array: IntArray): IntArrayDeque = IntArrayDeque(array, array.size)
    }
}

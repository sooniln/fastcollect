...
public class Long2ByteHashMap @JvmOverloads constructor(
...
) : AbstractMutableLong2ByteMap() {


    @JvmOverloads
    public constructor(map: Long2ByteMap, defaultValue: Byte = Byte.MIN_VALUE): this(defaultValue = defaultValue) { putAll(map) }
    @JvmOverloads
    public constructor(map: Map<Long, Byte>, defaultValue: Byte = Byte.MIN_VALUE): this(defaultValue = defaultValue) { putAll(map) }
    ...
    internal var valuesArr: ByteArray = EMPTY_VALUE_ARRAY
    ...
    override fun set(key: Long, value: Byte) {
        set(key, { value }, { value })
    }
    ...
    override fun removeKey(key: Long): Byte {
        return findSlot(
            key,
            { slot ->
                val oldValue = valuesArr[slot]
    ...
    override fun remove(key: Long, value: Byte): Boolean {
            return findSlot(
                key,
                { slot ->
                    val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Long, onAdd: () -> Byte, onReplace: (slot: Int) -> Byte) {
    ...
    private fun shiftAndInsert(keysArr: LongArray, mask: Int, slot: Int, currKey: Long, newKey: Long, newValue: Byte) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Long2ByteMap) {
        if (from is Long2ByteHashMap && from.size / 2 > size) {
    ...
    override fun putAll(from: Map<out Long, Byte>) {
        ensureCapacity(max(size + (from.size / 2), from.size))
    ...
    private fun resetTo(from: Long2ByteHashMap) {
        check(!from.isEmpty())
    ...
    private fun setRehashing(keysArr: LongArray, valuesArr: ByteArray, mask: Int, key: Long, value: Byte) {
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the Long2ByteHashMap while this
     * method is ongoing is not supported and leads to undefined behavior.
     */
    @JvmSynthetic
    public inline fun forEach(action: (Long2ByteMap.Entry) -> Unit) {
        val keysArr = keysArr
    ...
    internal class ReusableEntry(
        ...
        private val valuesArr: ByteArray,
    ...
    private inner class EntryIterator : MutableIterator<MutableLong2ByteMap.MutableEntry> {
        private val entry = MutableReusableEntry()

...
public class Long2ByteHashMap @JvmOverloads constructor(
    ...
    public constructor(map: Long2ByteMap): this() { putAll(map) }
    public constructor(map: Map<Long, Byte>): this() { putAll(map) }
    ...
    override val values: MutableByteCollection get() {
        return _values ?:
            ...
            object : MutableByteCollection {
                ...
                override fun foreach(action: ByteConsumer) = foreach { _, value -> action.accept(value) }
    ...
    override fun foreach(action: LongByteConsumer) {
        ...
        for (slot in keysArr.indices) {
            ...
            if (key != emptyKey) {
                @Suppress("UNCHECKED_CAST", "USELESS_CAST")
                action.accept(key, valuesArr[slot] as Byte)
            }
        }
    }

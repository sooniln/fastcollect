...
public class Int2LongHashMap @JvmOverloads constructor(
    ...
    @JvmOverloads
    public constructor(map: Int2LongMap, defaultValue: Long = Long.MIN_VALUE): this(defaultValue = defaultValue) { putAll(map) }
    @JvmOverloads
    ...
    override fun getValue(key: Int): Long = findSlot(key, { slot -> valuesArr[slot] }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST")
    override fun getOrDefault(key: Int, defaultValue: Long): Long = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })
    ...
    override fun putIfAbsent(key: Int, value: Long): Long {
    ...
    override fun replace(key: Int, value: Long): Long {
    ...
    override fun remove(key: Int, value: Long): Boolean {
            return findSlot(
                key,
                { slot ->
                    val oldValue = valuesArr[slot]
    ...
    private fun shiftAndInsert(keysArr: IntArray, mask: Int, slot: Int, currKey: Int, newKey: Int, newValue: Long) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Int2LongMap) {
        if (from is Int2LongHashMap && from.size / 2 > size) {
    ...
    private fun resetTo(from: Int2LongHashMap) {
        check(!from.isEmpty())
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the Int2LongHashMap while this

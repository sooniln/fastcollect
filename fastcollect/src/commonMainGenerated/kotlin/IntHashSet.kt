...
public class IntHashSet @JvmOverloads constructor(
    ...
    @JvmOverloads
    public constructor(array: IntArray, fromIndex: Int = 0, toIndex: Int = array.size): this() { addAll(array, fromIndex, toIndex) }

    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var keysArr: IntArray = EMPTY_ARRAY
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the IntHashSet while this
...
public fun IntHashSet.removeAll(predicate: IntPredicate): Boolean = filterInPlace { predicate.test(it) }
public fun IntHashSet.retainAll(predicate: IntPredicate): Boolean = filterInPlace { !predicate.test(it) }

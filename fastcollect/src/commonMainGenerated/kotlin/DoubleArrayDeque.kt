...
/**
 * An array based [Deque](https://en.wikipedia.org/wiki/Double-ended_queue) implementation for storing Doubles.
 *
...
public class DoubleArrayDeque private constructor(array: DoubleArray, size: Int) : AbstractMutableDoubleList() {
    ...
    private fun grow(capacity: Int) {
        ...
        if (head == 0) {
            ...
            ring = copyIntoInternal(DoubleArray(newCapacity))
            head = 0
        }
    }
    ...
    public fun trimToSize() {
        if (size < ring.size) {
            ring = if (isEmpty()) EMPTY_ARRAY else copyIntoInternal(DoubleArray(size))
            head = 0
        }
    }
    ...
    public override fun removeAll(elements: DoubleCollection): Boolean {
    ...
    public override fun retainAll(elements: DoubleCollection): Boolean {
        return filterInPlace { e -> !elements.contains(e) }
    }
    ...
    private fun copyIntoInternal(dest: DoubleArray, destinationOffset: Int = 0, fromIndex: Int = 0, toIndex: Int = size): DoubleArray {
    ...
    internal companion object {
        ...
        private fun DoubleArray.positiveMod(position: Int): Int = if (position < size) position else position - size

        private fun DoubleArray.negativeMod(position: Int): Int = if (position < 0) position + size else position

        private fun DoubleArray.position(head: Int, index: Int): Int = positiveMod(head + index)

        private fun DoubleArray.index(head: Int, position: Int): Int = negativeMod(position - head)

        private fun DoubleArray.incrementPosition(position: Int): Int {
            val next = position + 1

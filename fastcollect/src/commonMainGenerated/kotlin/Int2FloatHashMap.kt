...
public class Int2FloatHashMap @JvmOverloads constructor(
    ...
    override fun getValue(key: Int): Float = findSlot(key, { slot -> valuesArr[slot] as Float }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getOrDefault(key: Int, defaultValue: Float): Float = findSlot(key, { slot -> valuesArr[slot] as Float }, { defaultValue })
    ...
    private inner class MutableReusableEntry : MutableInt2FloatMap.AbstractMutableEntry() {
        ...
        override var value: Float
            get() {
                check(previousSlot != -1)
                return valuesArr[previousSlot] as Float
            }
            set(value) {
                check(previousSlot != -1)
                if (keysArr !== this@Int2FloatHashMap.keysArr) throw ConcurrentModificationException()

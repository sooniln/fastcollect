...
public class Int2FloatHashMap @JvmOverloads constructor(
    ...
    private val defaultValue: Float = Float.NaN,

) : AbstractMutableInt2FloatMap() {


    @JvmOverloads
    public constructor(map: Int2FloatMap, defaultValue: Float = Float.NaN): this(defaultValue = defaultValue) { putAll(map) }
    @JvmOverloads
    public constructor(map: Map<Int, Float>, defaultValue: Float = Float.NaN): this(defaultValue = defaultValue) { putAll(map) }
    ...
    internal var valuesArr: FloatArray = EMPTY_VALUE_ARRAY
    ...
    override fun isDefaultValue(value: Float): Boolean = value equalsRaw defaultValue
    ...
    override fun get(key: Int): Float = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })

    @Suppress("UNCHECKED_CAST")
    override fun getValue(key: Int): Float = findSlot(key, { slot -> valuesArr[slot] }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST")
    override fun getOrDefault(key: Int, defaultValue: Float): Float = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })
    ...
    override fun putIfAbsent(key: Int, value: Float): Float {
    ...
    override fun replace(key: Int, value: Float): Float {
    ...
    override fun remove(key: Int, value: Float): Boolean {
            return findSlot(
                key,
                { slot ->
                    val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Int, onAdd: () -> Float, onReplace: (slot: Int) -> Float) {
    ...
    private fun shiftAndInsert(keysArr: IntArray, mask: Int, slot: Int, currKey: Int, newKey: Int, newValue: Float) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Int2FloatMap) {
        if (from is Int2FloatHashMap && from.size / 2 > size) {
    ...
    private fun resetTo(from: Int2FloatHashMap) {
        check(!from.isEmpty())
    ...
    private var _values: FloatCollection? = null

    @get:JvmName("values")
    override val values: FloatCollection get() {
        return _values ?:

            object : AbstractFloatCollection() {

                override val size: Int get() = this@Int2FloatHashMap.size
                override fun contains(element: Float): Boolean = containsValue(element)
                override fun iterator(): FloatIterator = ValueIterator()
            }
            .also { _values = it }
    }
    ...
    private fun setRehashing(keysArr: IntArray, valuesArr: FloatArray, mask: Int, key: Int, value: Float) {
    ...
    /**
     * Guaranteed to be as fast or faster than using [iterator] to iterate. Modifying the Int2FloatHashMap while this
     * method is ongoing is not supported and leads to undefined behavior.
     */
    @JvmSynthetic
    public inline fun forEach(action: (Int2FloatMap.Entry) -> Unit) {
        val keysArr = keysArr
    ...
    internal class ReusableEntry(
        ...
        private val valuesArr: FloatArray,

    ) : Int2FloatMap.AbstractEntry() {
        @set:JvmSynthetic
        override var key: Int = 0.toInt()
        @Suppress("UNCHECKED_CAST")
        override val value: Float get() = valuesArr[slot]
    }

    private inner class MutableReusableEntry : MutableInt2FloatMap.AbstractMutableEntry() {
    ...
    private inner class ValueIterator : FloatIterator() {
    ...
    private inner class EntryIterator : MutableIterator<MutableInt2FloatMap.MutableEntry> {
        private val entry = MutableReusableEntry()
    ...
    private companion object {
        ...
        private val EMPTY_VALUE_ARRAY = FloatArray(1)

...
/**
 * An array based [Deque](https://en.wikipedia.org/wiki/Double-ended_queue) implementation for storing Floats. Can be
...
public class FloatArrayDeque private constructor(array: FloatArray, size: Int = array.size) : AbstractMutableFloatList(), RandomAccess {
    ...
    private fun foreachContinuous(ring: FloatArray, tail: Int, action: FloatConsumer) {
        var position = head
        while (position < tail) {
    ...
    private fun foreachDiscrete(ring: FloatArray, action: FloatConsumer) {
        var remaining = size
        var position = head
        while (remaining > 0) {
            action.accept(ring[position])
    ...
    private fun FloatArray.position(index: Int): Int = positiveMod(head + index)

    private fun FloatArray.index(position: Int): Int = negativeMod(position - head)

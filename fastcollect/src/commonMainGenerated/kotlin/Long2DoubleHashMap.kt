...
public class Long2DoubleHashMap @JvmOverloads constructor(
    capacity: Int = 0,
    ...
    @JvmOverloads
    public constructor(map: Long2DoubleMap, defaultValue: Double = Double.NaN): this(defaultValue = defaultValue) { putAll(map) }
    @JvmOverloads
    public constructor(map: Map<Long, Double>, defaultValue: Double = Double.NaN): this(defaultValue = defaultValue) { putAll(map) }


    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var keysArr: LongArray = EMPTY_KEY_ARRAY

    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic

    internal var valuesArr: DoubleArray = EMPTY_VALUE_ARRAY


    @PublishedApi
    @get:JvmSynthetic
    @set:JvmSynthetic
    internal var emptyKey: Long = ZERO
    ...
    override fun isDefaultValue(value: Double): Boolean = value equalsRaw defaultValue
    ...
    override fun get(key: Long): Double = findSlot(key, { slot -> valuesArr[slot] }, { defaultValue })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getValue(key: Long): Double = findSlot(key, { slot -> valuesArr[slot] as Double }, { throw NoSuchElementException() })

    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
    override fun getOrDefault(key: Long, defaultValue: Double): Double = findSlot(key, { slot -> valuesArr[slot] as Double }, { defaultValue })
    ...
    override fun putIfAbsent(key: Long, value: Double): Double {
    ...
    override fun replace(key: Long, value: Double): Double {
    ...
    override fun remove(key: Long, value: Double): Boolean {
            return findSlot(
                key,
                { slot ->
                    val oldValue = valuesArr[slot]
    ...
    private inline fun set(key: Long, onAdd: () -> Double, onReplace: (slot: Int) -> Double) {
    ...
    private fun shiftAndInsert(keysArr: LongArray, mask: Int, slot: Int, currKey: Long, newKey: Long, newValue: Double) {
        val valuesArr = valuesArr
    ...
    override fun putAll(from: Long2DoubleMap) {
        if (from is Long2DoubleHashMap && from.size / 2 > size) {
            ...
            for (slot in oldKeysArr.indices) {
                ...
                if (key != oldEmptyKey) {
                    @Suppress("UNCHECKED_CAST", "USELESS_CAST")
                    putIfAbsent(key, oldValuesArr[slot] as Double)
                }
            }
        } else {
    ...
    private fun resetTo(from: Long2DoubleHashMap) {
        check(!from.isEmpty())
    ...
    private var _values: DoubleCollection? = null

    @get:JvmName("values")
    override val values: DoubleCollection get() {
        return _values ?:

            object : AbstractDoubleCollection() {

                override val size: Int get() = this@Long2DoubleHashMap.size
                override fun contains(element: Double): Boolean = containsValue(element)
                override fun iterator(): DoubleIterator = ValueIterator()
            }
            .also { _values = it }
    }
    ...
    private fun setRehashing(keysArr: LongArray, valuesArr: DoubleArray, mask: Int, key: Long, value: Double) {
    ...
    internal class ReusableEntry(
        ...
        private val valuesArr: DoubleArray,

    ) : Long2DoubleMap.AbstractEntry() {
        @set:JvmSynthetic
        override var key: Long = 0.toLong()
        @Suppress("UNCHECKED_CAST", "USELESS_CAST")
        override val value: Double get() = valuesArr[slot] as Double
    }

    private inner class MutableReusableEntry : MutableLong2DoubleMap.AbstractMutableEntry() {
        private val keysArr = this@Long2DoubleHashMap.keysArr
        private val emptyKey = this@Long2DoubleHashMap.emptyKey
        ...
        override var value: Double
            get() {
                check(previousSlot != -1)
                return valuesArr[previousSlot] as Double
            }
            set(value) {
                check(previousSlot != -1)
                if (keysArr !== this@Long2DoubleHashMap.keysArr) throw ConcurrentModificationException()
    ...
    private inner class ValueIterator : DoubleIterator() {
    ...
    private inner class EntryIterator : MutableIterator<MutableLong2DoubleMap.MutableEntry> {
        private val entry = MutableReusableEntry()
    ...
    private companion object {
        ...
        private val EMPTY_VALUE_ARRAY = DoubleArray(1)

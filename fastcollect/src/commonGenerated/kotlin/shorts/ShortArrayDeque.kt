...
/**
 * An array based [Deque](https://en.wikipedia.org/wiki/Double-ended_queue) implementation for storing {Short}s. Can be
...
public class ShortArrayDeque private constructor(array: ShortArray, size: Int = array.size) : AbstractMutableShortList(), RandomAccess {
    ...
    private fun removeMiddle(index: Int): Short {
        // attempt to shift a minimal number of elements depending on where position falls within the array
        val position = ring.position(index)
    ...
    override fun iterator(): MutableShortIterator = DequeIterator()
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (other is ShortList && other is RandomAccess) {
            for (i in indices) {
            ...
            var i = 0
            if (it is ShortIterator) {
                while (it.hasNext()) {
                    if (it.nextShort() != this.getAt(i++)) return false
                }
            } else {
                while (it.hasNext()) {
    ...
    private inner class DequeIterator : MutableShortIterator() {

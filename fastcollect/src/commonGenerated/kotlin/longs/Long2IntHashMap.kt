...
public class Long2IntHashMap(
    ...
    public constructor(map: Map<Long, Int>): this(map.size) {
        putAll(map)
    }
    ...
    public fun trimToSize() {
        ...
        if (newLength > HASHIFY_THRESHOLD) {
            ...
            if (oldKeys[oldEndSlot] == ZERO) {
                putArray(ZERO, oldValues[oldEndSlot] as Int)
            }
        }
        ...
        for (slot in 0..<oldEndSlot) {
            ...
            if (key != ZERO) {
                putHashing(key, oldValues[slot] as Int)
            }
        }
    ...
    private fun putHashing(key: Long, value: Int): Int {
        val keysArr = keysArr
    ...
    private fun putArray(key: Long, value: Int): Int {
        var slot = 0
        while (slot < size) {
            if (keysArr[slot] == key) {
                val oldValue = valuesArr[slot]
    ...
    private var _primitiveEntries: MutableEntrySet<MutableLong2IntMap.MutableEntry>? = null
    override val primitiveEntries: MutableEntrySet<MutableLong2IntMap.MutableEntry> get() {
    ...
    private fun containsValueHashing(value: Int): Boolean {
        val endSlot = keysArr.endSlot()
    ...
    private fun containsValueArray(value: Int): Boolean {
        var slot = size - 1
        while (slot >= 0) {
    ...
    private fun growTo(capacity: Int) {
        ...
        if (!oldKeys.isHashing()) {
            ...
            var slot = 0
            while (slot < oldSize) {
                putHashing(oldKeys[slot], oldValues[slot] as Int)
                ++slot
            }
        } else {
    ...
    private open inner class SlotIterator {
        val keysArr = this@Long2IntHashMap.keysArr
        val valuesArr = this@Long2IntHashMap.valuesArr
    ...
    private inner class EntryIterator : SlotIterator(), MutableIterator<MutableLong2IntMap.MutableEntry> {

        override fun next(): MutableLong2IntMap.MutableEntry {
            nextSlot()
            return object : MutableLong2IntMap.MutableEntry {

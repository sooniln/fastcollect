...
public class LongHashSet @JvmOverloads constructor(
    ...
    override fun addAll(elements: Collection<Long>): Boolean {
        ...
        if (elements is LongCollection) {
            if (isEmpty() && elements is LongHashSet) {
    ...
    private fun initializeFrom(from: LongHashSet) {
        check(isEmpty())
    ...
    private fun addRehashing(keysArr: LongArray, mask: Int, rotVal: Int, key: Long) {
        var slot = key.slot(mask, rotVal)
        var newKey = key
        var newKeySlotDistance = 0
        while (true) {
    ...
    private companion object {
        ...
        private val EMPTY_ARRAY = longArrayOf(ZERO, NONZERO)

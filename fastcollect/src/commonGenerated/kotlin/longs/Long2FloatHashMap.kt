...
public class Long2FloatHashMap(
    ...
    public fun trimToSize() {
        ...
        if (newLength > HASHIFY_THRESHOLD) {
            ...
            if (oldKeys[oldEndSlot] == ZERO) {
                putArray(ZERO, oldValues[oldEndSlot] as Float)
            }
        }
        ...
        for (slot in 0..<oldEndSlot) {
            ...
            if (key != ZERO) {
                putHashing(key, oldValues[slot] as Float)
            }
        }
    ...
    private fun putHashing(key: Long, value: Float): Float {
        val keysArr = keysArr
    ...
    private fun putArray(key: Long, value: Float): Float {
        var slot = 0
        while (slot < size) {
            if (keysArr[slot] == key) {
                val oldValue = valuesArr[slot]
    ...
    private fun growTo(capacity: Int) {
        ...
        if (!oldKeys.isHashing()) {
            ...
            var slot = 0
            while (slot < oldSize) {
                putHashing(oldKeys[slot], oldValues[slot] as Float)
                ++slot
            }
        } else {

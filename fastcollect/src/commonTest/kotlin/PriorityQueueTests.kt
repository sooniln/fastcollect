...
class BytePriorityQueueTest {
    @Test
    fun add_removeFirst_returnsAscendingOrder() {
        val queue = BytePriorityQueue()
        for (v in listOf(5, 3, 8, 1, 9, 2)) queue.add(v.toByte())

        assertEquals(6, queue.size)
        for (expected in listOf(1, 2, 3, 5, 8, 9)) {
            assertEquals(expected.toByte(), queue.removeFirst())
        }
    ...
    fun descending_removeFirst_returnsDescendingOrder() {
        val queue = BytePriorityQueue(descending = true)
        for (v in listOf(5, 3, 8, 1, 9, 2)) queue.add(v.toByte())
    ...
    fun first_doesNotRemove() {
        val queue = bytePriorityQueueOf(4.toByte(), 2.toByte(), 6.toByte())
        assertEquals(2.toByte(), queue.first())
    ...
    fun first_emptyQueue_throws() {
        assertFailsWith<NoSuchElementException> { BytePriorityQueue().first() }
    }

    @Test
    fun removeFirst_emptyQueue_throws() {
        assertFailsWith<NoSuchElementException> { BytePriorityQueue().removeFirst() }
    }

    @Test
    fun priorityQueueOf_vararg() {
        val queue = bytePriorityQueueOf(7.toByte(), 1.toByte(), 4.toByte())
        assertEquals(3, queue.size)
        assertEquals(1.toByte(), queue.removeFirst())
        assertEquals(4.toByte(), queue.removeFirst())
        assertEquals(7.toByte(), queue.removeFirst())
    }

    @Test
    fun buildPriorityQueue_dsl() {
        val queue = buildBytePriorityQueue {
            add(10.toByte())
            add(5.toByte())
            add(20.toByte())
        }
        assertEquals(5.toByte(), queue.removeFirst())
        assertEquals(10.toByte(), queue.removeFirst())
        assertEquals(20.toByte(), queue.removeFirst())
    }

    @Test
    fun constructor_fromCollection_heapifies() {
        val source = mutableByteListOf(6.toByte(), 1.toByte(), 4.toByte(), 2.toByte(), 9.toByte())
        val queue = BytePriorityQueue(source)
    ...
    fun contains_findsPresentElements() {
        val queue = bytePriorityQueueOf(3.toByte(), 7.toByte(), 1.toByte(), 9.toByte())
        assertTrue(queue.contains(7.toByte()))
        assertFalse(queue.contains(42.toByte()))
    }

    @Test
    fun remove_nonRootElement_preservesHeapOrder() {
        val queue = bytePriorityQueueOf(5.toByte(), 3.toByte(), 8.toByte(), 1.toByte(), 9.toByte(), 2.toByte(), 7.toByte())
        assertTrue(queue.remove(8.toByte()))
        assertFalse(queue.contains(8.toByte()))
        assertEquals(6, queue.size)

        val drained = mutableListOf<Byte>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 2, 3, 5, 7, 9).map { it.toByte() }, drained)
    }

    @Test
    fun remove_missingElement_returnsFalse() {
        val queue = bytePriorityQueueOf(1.toByte(), 2.toByte(), 3.toByte())
        assertFalse(queue.remove(42.toByte()))
        assertEquals(3, queue.size)
    }
    ...
    fun removeAll_removesMatchingElements_preservesHeapOrder() {
        val queue = bytePriorityQueueOf(5.toByte(), 3.toByte(), 8.toByte(), 1.toByte(), 9.toByte(), 2.toByte(), 7.toByte())
        val modified = queue.removeAll(mutableByteListOf(3.toByte(), 9.toByte()))

        assertTrue(modified)
        assertEquals(5, queue.size)
        val drained = mutableListOf<Byte>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 2, 5, 7, 8).map { it.toByte() }, drained)
    }

    @Test
    fun removeAll_withBoxedCollection() {
        val queue = bytePriorityQueueOf(5.toByte(), 3.toByte(), 8.toByte(), 1.toByte())
        val modified = queue.removeAll(listOf(3.toByte(), 1.toByte()))

        assertTrue(modified)
        assertEquals(listOf(5, 8).map { it.toByte() }, listOf(queue.removeFirst(), queue.removeFirst()))
    }
    ...
    fun removeAll_withPredicate_removesMatchingElements() {
        ...
        val drained = mutableListOf<Byte>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 2, 3, 5).map { it.toByte() }, drained)
    }

    @Test
    fun retainAll_keepsOnlyMatchingElements_preservesHeapOrder() {
        val queue = bytePriorityQueueOf(5.toByte(), 3.toByte(), 8.toByte(), 1.toByte(), 9.toByte(), 2.toByte(), 7.toByte())
        val modified = queue.retainAll(mutableByteListOf(3.toByte(), 9.toByte(), 1.toByte()))

        assertTrue(modified)
        assertEquals(3, queue.size)
        val drained = mutableListOf<Byte>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 3, 9).map { it.toByte() }, drained)
    }
    ...
    fun ensureCapacity_doesNotLoseElements() {
        ...
        assertEquals(3, queue.size)
        assertEquals(
            listOf(1, 2, 3).map { it.toByte() },
            listOf(queue.removeFirst(), queue.removeFirst(), queue.removeFirst()),
        )
    }

    @Test
    fun trimToSize_doesNotLoseElements() {
        val queue = BytePriorityQueue(100).apply { add(3.toByte()); add(1.toByte()); add(2.toByte()) }
        queue.trimToSize()
        assertEquals(3, queue.size)
        assertEquals(
            listOf(1, 2, 3).map { it.toByte() },
    ...
    fun iterator_visitsAllElements() {
        val queue = bytePriorityQueueOf(5.toByte(), 3.toByte(), 8.toByte(), 1.toByte())
        val visited = mutableListOf<Byte>()
        val it = queue.iterator()
        while (it.hasNext()) visited.add(it.nextByte())

        assertEquals(listOf(5, 3, 8, 1).map { it.toByte() }.sorted(), visited.sorted())
    }
...
class ShortPriorityQueueTest {
    @Test
    fun add_removeFirst_returnsAscendingOrder() {
        val queue = ShortPriorityQueue()
        for (v in listOf(5, 3, 8, 1, 9, 2)) queue.add(v.toShort())

        assertEquals(6, queue.size)
        for (expected in listOf(1, 2, 3, 5, 8, 9)) {
            assertEquals(expected.toShort(), queue.removeFirst())
        }
        assertTrue(queue.isEmpty())
    }
    ...
    @Test
    fun descending_removeFirst_returnsDescendingOrder() {
        val queue = ShortPriorityQueue(descending = true)
        for (v in listOf(5, 3, 8, 1, 9, 2)) queue.add(v.toShort())
    ...
    @Test
    fun first_doesNotRemove() {
        val queue = shortPriorityQueueOf(4.toShort(), 2.toShort(), 6.toShort())
        assertEquals(2.toShort(), queue.first())
        assertEquals(3, queue.size)
        assertEquals(2.toShort(), queue.first())
    }
    ...
    @Test
    fun first_emptyQueue_throws() {
        assertFailsWith<NoSuchElementException> { ShortPriorityQueue().first() }
    }

    @Test
    fun removeFirst_emptyQueue_throws() {
        assertFailsWith<NoSuchElementException> { ShortPriorityQueue().removeFirst() }
    }

    @Test
    fun priorityQueueOf_vararg() {
        val queue = shortPriorityQueueOf(7.toShort(), 1.toShort(), 4.toShort())
        assertEquals(3, queue.size)
        assertEquals(1.toShort(), queue.removeFirst())
        assertEquals(4.toShort(), queue.removeFirst())
        assertEquals(7.toShort(), queue.removeFirst())
    }

    @Test
    fun buildPriorityQueue_dsl() {
        val queue = buildShortPriorityQueue {
            add(10.toShort())
            add(5.toShort())
            add(20.toShort())
        }
        assertEquals(5.toShort(), queue.removeFirst())
        assertEquals(10.toShort(), queue.removeFirst())
        assertEquals(20.toShort(), queue.removeFirst())
    }

    @Test
    fun constructor_fromCollection_heapifies() {
        val source = mutableShortListOf(6.toShort(), 1.toShort(), 4.toShort(), 2.toShort(), 9.toShort())
        val queue = ShortPriorityQueue(source)
        assertEquals(5, queue.size)
        for (expected in listOf(1, 2, 4, 6, 9)) {
            assertEquals(expected.toShort(), queue.removeFirst())
        }
    ...
    @Test
    fun contains_findsPresentElements() {
        val queue = shortPriorityQueueOf(3.toShort(), 7.toShort(), 1.toShort(), 9.toShort())
        assertTrue(queue.contains(7.toShort()))
        assertFalse(queue.contains(42.toShort()))
    }

    @Test
    fun remove_nonRootElement_preservesHeapOrder() {
        val queue = shortPriorityQueueOf(
            5.toShort(), 3.toShort(), 8.toShort(), 1.toShort(), 9.toShort(), 2.toShort(), 7.toShort(),
        )
        assertTrue(queue.remove(8.toShort()))
        assertFalse(queue.contains(8.toShort()))
        assertEquals(6, queue.size)

        val drained = mutableListOf<Short>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 2, 3, 5, 7, 9).map { it.toShort() }, drained)
    }

    @Test
    fun remove_missingElement_returnsFalse() {
        val queue = shortPriorityQueueOf(1.toShort(), 2.toShort(), 3.toShort())
        assertFalse(queue.remove(42.toShort()))
        assertEquals(3, queue.size)
    }
    ...
    fun removeAll_removesMatchingElements_preservesHeapOrder() {
        ...
        val modified = queue.removeAll(mutableShortListOf(3.toShort(), 9.toShort()))

        assertTrue(modified)
        assertEquals(5, queue.size)
        val drained = mutableListOf<Short>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 2, 5, 7, 8).map { it.toShort() }, drained)
    }

    @Test
    fun removeAll_withBoxedCollection() {
        val queue = shortPriorityQueueOf(5.toShort(), 3.toShort(), 8.toShort(), 1.toShort())
        val modified = queue.removeAll(listOf(3.toShort(), 1.toShort()))

        assertTrue(modified)
        assertEquals(listOf(5, 8).map { it.toShort() }, listOf(queue.removeFirst(), queue.removeFirst()))
    }
    ...
    fun removeAll_withPredicate_removesMatchingElements() {
        ...
        val drained = mutableListOf<Short>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 2, 3, 5).map { it.toShort() }, drained)
    }
    ...
    fun retainAll_keepsOnlyMatchingElements_preservesHeapOrder() {
        ...
        val modified = queue.retainAll(mutableShortListOf(3.toShort(), 9.toShort(), 1.toShort()))

        assertTrue(modified)
        assertEquals(3, queue.size)
        val drained = mutableListOf<Short>()
        while (!queue.isEmpty()) drained.add(queue.removeFirst())
        assertEquals(listOf(1, 3, 9).map { it.toShort() }, drained)
    }
    ...
    @Test
    fun trimToSize_doesNotLoseElements() {
        val queue = ShortPriorityQueue(100).apply { add(3.toShort()); add(1.toShort()); add(2.toShort()) }
        queue.trimToSize()
        assertEquals(3, queue.size)
        assertEquals(
            listOf(1, 2, 3).map { it.toShort() },
    ...
    fun iterator_visitsAllElements() {
        val queue = shortPriorityQueueOf(5.toShort(), 3.toShort(), 8.toShort(), 1.toShort())
        val visited = mutableListOf<Short>()
        val it = queue.iterator()
        while (it.hasNext()) visited.add(it.nextShort())

        assertEquals(listOf(5, 3, 8, 1).map { it.toShort() }.sorted(), visited.sorted())
    }

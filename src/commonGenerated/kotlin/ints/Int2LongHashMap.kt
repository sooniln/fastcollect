...
internal class Int2LongHashMap(
    ...
    private fun putInternalHashing(key: Int, value: Long): Long {
        // assert(isHashing())
        // assert(key != 0)
    ...
    private fun putInternalArray(key: Int, value: Long): Long {
        // assert(!isHashing())
        // assert(key != 0)
    ...
    override val primitiveEntries: MutableSet<MutableInt2LongMap.MutableEntry> by lazy {
        object : AbstractMutableSet<MutableInt2LongMap.MutableEntry>() {
            override val size: Int get() = this@Int2LongHashMap.size
            override fun contains(element: MutableInt2LongMap.MutableEntry): Boolean {
                val value = getValue(element.key)
                return if (value == defaultValue && !containsKey(element.key)) false else value == element.value
            }
            override fun add(element: MutableInt2LongMap.MutableEntry): Boolean = throw UnsupportedOperationException()
            override fun remove(element: MutableInt2LongMap.MutableEntry): Boolean = throw UnsupportedOperationException()
            override fun iterator(): MutableIterator<MutableInt2LongMap.MutableEntry> = EntryIterator<MutableInt2LongMap.MutableEntry> { key, value -> Entry(key, value) }
            override fun clear() = this@Int2LongHashMap.clear()
        }
    }
    ...
    private inner class KeyIterator : MutableIntIterator() {
        ...
        private val arrayUsage = this@Int2LongHashMap.arrayUsage
    ...
    private inner class EntryIterator<T>(private val visitor: (Int, Long) -> T): MutableIterator<T> {
        private val keysArr = this@Int2LongHashMap.keysArr
        private val valuesArr = this@Int2LongHashMap.valuesArr
        private val arrayUsage = this@Int2LongHashMap.arrayUsage
    ...
    private inner class Entry(override val key: Int, value: Long) : MutableInt2LongMap.MutableEntry {
